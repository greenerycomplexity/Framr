            ...
            ScrollViewReader { proxy in
                ScrollView(.horizontal, showsIndicators: false) {
                    LazyHStack(spacing: frameSpacing) {
                        ForEach(0..<playerManager.totalFrameCount, id: \.self) {
                            frameIndex in
                            FrameThumbnailView(
                            ...
                            .scaleEffect(
                                !isDragging && centerFrameIndex == frameIndex
                                    ? 1.15 : 0.9
                            )
                            .padding(
                                .horizontal,
                ...
                .onPreferenceChange(FrameOffsetPreferenceKey.self) {
                    frameOffsets in
                    ...
                    if centerFrameIndex != closestFrame {
                        ...
                        if isUserScrolling && lastSeekFrame != closestFrame
                ...
                .onAppear {
                    ...
                    if centerFrameIndex == 0 {
                        ...
                        proxy.scrollTo(
                            playerManager.currentFrameIndex,
                            anchor: .center
                        )
                    }
                }
                .onChange(of: playerManager.currentFrameIndex) {
                    oldValue,
                    newValue in
                    ...
                    if let lastCarouselSeek = lastCarouselSeekTime {
                        let timeSinceCarouselSeek = Date().timeIntervalSince(
                            lastCarouselSeek
                        )
                    ...
                    if let lastInteraction = lastUserInteractionTime {
                        let timeSinceInteraction = Date().timeIntervalSince(
                            lastInteraction
                        )
                ...
                .onChange(of: playerManager.isPlaying) { oldValue, newValue in
                    // When video is paused, sync carousel to current frame
                    if oldValue == true && newValue == false {
                        let currentFrame = playerManager.currentFrameIndex
